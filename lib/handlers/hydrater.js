...
/**
 * This handler receives a document on a POST request and processes it.
 *
...
module.exports = function(req, res, server, logger, next) {
  if(!req.params.file_path) {
    return next(new restify.MissingParameterError('No file to process'));
  ...
  task.document.metadata = task.document.metadata || {};
  task.document.data = task.document.data || {};
