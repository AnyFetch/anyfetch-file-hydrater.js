...
process.on('message', function(task) {
  ...
  logError.config = task.config.opbeat;

  /* istanbul ignore next */
  if(!logError.opbeat && task.opbeatConfig && task.opbeatConfig.secretToken) {
    var opbeat = require('opbeat');
    logError.opbeat = opbeat(task.config.opbeat);
  }
  ...
  async.waterfall([
    ...
    function downloadFile(cb) {
      if(task.file_path) {
        ...
        if(task.config.deniedPorts && task.config.deniedPorts.indexOf(apiUrl.port) !== -1) {
          stream.end();
          return fs.unlink(path, function() {
            cb(new Error('Trying to access to a denied port: ' + apiUrl.port));
          });
        }
