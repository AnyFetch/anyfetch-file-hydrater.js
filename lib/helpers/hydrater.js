...
var changes = require('../index.js').defaultChanges();
...
module.exports = function(hydraterFunction, logger) {
  ...
  return function(task, done) {
    ...
    async.waterfall([
      ...
      function startHydration(path, cb) {
        ...
        domain.run(function() {
          var onCb = function(err, changes) {
            ...
            cb(null, err, path, changes, domain);
          };
      ...
      function removeFile(err, path, changes, domain, cb) {
        // Clean domain
        domain.exit();
        domain.dispose();
        // Remove file from fs
        fs.unlink(path, function(_err) {
          // forward original error if any, after cleanup
          if(err) {
            cb(err);
          }
