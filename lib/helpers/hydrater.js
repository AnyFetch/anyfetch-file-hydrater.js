  ...
  return function(task, done) {
    ...
    async.waterfall([
      function initHydration(cb) {
        ...
        req.piped = true; // HACK...
        req.buffer(false);
        req.end().req.on('response', function(res){
          if(res.statusCode !== 200) {
            return cb(new Error("Invalid statusCode (" + res.statusCode + ") while downloading file," + JSON.stringify(res)));
          }
          if (/^(deflate|gzip)$/.test(res.headers['content-encoding'])) {
            res.pipe(zlib.createUnzip()).pipe(stream);
          } else {
            res.pipe(stream);
          }
          cb(null, path);
